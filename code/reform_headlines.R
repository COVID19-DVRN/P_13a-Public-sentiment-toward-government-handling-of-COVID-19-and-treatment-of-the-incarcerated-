...
raw_stories <- read_csv(here::here("data", "criminal justice reform searches",
                                   "reform search6.csv"),
                        col_types = cols(
  ...
  mutate(file = "reform search6.csv")
...
#   select(title, date, duplicate, url, source_group, media_name) %>%
#   arrange(title) %>% 
#   view()
#
...
write_rds(stories_no_dupes, here::here("data", "reform_full.rds"))
write_csv(stories_no_dupes, here::here("data", "reform_full.csv"))
