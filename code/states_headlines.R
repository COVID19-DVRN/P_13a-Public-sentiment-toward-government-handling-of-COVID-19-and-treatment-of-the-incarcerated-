...
raw_stories <- read_csv(mediacloud_csv_path) %>%
  mutate(file = sub("^.*\\/(.+)\\.csv", "\\1", mediacloud_csv_path))

# mediacloud sources
files <- list.files(here::here("data", "sources", "states"),
                    full.names = TRUE, pattern = "*.csv")
states_order <- tibble(source_group = as.character(1:length(files)),
                       state = map_chr(files, ~str_split(., "\\(|\\_\\_")[[1]][2] %>%
                          str_replace_all("\\_", " ")))
...
sources <- map_dfr(files, read_csv,
  ...
  left_join(states_order, by = "source_group") %>%
  select(-source_group) %>%
  arrange(state) %>%
  group_by_at(vars(-state)) %>%
  mutate(duplicate_source = n() > 1) %>% # some sources belong to > 1 group
  ungroup()

# merge with stories (duplicates some due to multiple states per source)
stories <- raw_stories %>%
...
# keep only one of the same story per date, state
stories_no_dupes <- stories %>%
  group_by(date, raw_title, state) %>% # remove duplicate titles w/out punctuation, capitals
  arrange(publish_date, -ap_syndicated) %>%
  mutate(n = n(), duplicate = n() > 1) %>%
  filter(row_number() == 1) %>%
  ungroup()

# # confirm maually that the duplicates really were duplicates
# dupes <- stories_no_dupes %>% filter(duplicate)
# other_dupes <- stories %>% anti_join(stories_no_dupes)
# 
# bind_rows(dupes, other_dupes) %>%
#   select(title, date, duplicate, url, state, media_name) %>%
#   arrange(title) %>%
#   view()
#
# # and manually find more duplicates
...
# duplicate_stories <- c(
#   1555628762, 1503691627, 	1561626035, 	1557816508, 1582701278, 1517295755
# )
# 
# stories_no_dupes <- stories_no_dupes %>%
...
already_downloaded <- str_extract(
  list.files(here::here("data", "all_full_xml")), 
  "[0-9]+")

stories_no_dupes$can_skip <- stories_no_dupes$stories_id %in% 
  parse_number(already_downloaded)

skipped <- stories_no_dupes %>%
  filter(can_skip)

to_download <- stories_no_dupes %>%
  filter(!can_skip)

full_texts_downloaded <- to_download %>%
  pull(url) %>% 
  map(safe_full_text) %>%
  set_names(to_download$stories_id)

safe_write_xml <- possibly(write_xml, otherwise = NA)

walk2(full_texts_downloaded, to_download$stories_id,
      ~safe_write_xml(.x, file = str_glue(here::here("data", "all_full_xml",
                                                     "text_{.y}.xml"))))

full_texts_skipped <- map(skipped$stories_id, 
                          ~safe_full_text(here::here("data", "all_full_xml",
                                                     str_glue("text_{.}.xml")))) %>%
  set_names(skipped$stories_id)

full_texts_html <- c(full_texts_skipped, full_texts_downloaded)
...
full_texts_df <- tibble(stories_id = parse_number(names(full_texts)),
                        full_text = full_texts)

stories_no_dupes <- stories_no_dupes %>%
  left_join(full_texts_df) %>%
  mutate(full_text = map_chr(full_text, ~ reduce(., paste, .init = "")))
